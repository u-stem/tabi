...
describe("EmailSection", () => {
  ...
  it("email が @sugara.local の場合「未設定」と表示する", () => {
    renderWithIntl(<EmailSection currentEmail="alice@sugara.local" emailVerified={false} />);
    expect(screen.getByText("未設定")).toBeDefined();
  });

  it("実アドレスかつ verified=true の場合マスクして表示する", () => {
    renderWithIntl(<EmailSection currentEmail="alice@gmail.com" emailVerified={true} />);
  ...
  it("実アドレスかつ verified=false の場合「未確認」バッジを表示する", () => {
    renderWithIntl(<EmailSection currentEmail="alice@gmail.com" emailVerified={false} />);
