# Messages Consolidation Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Consolidate all user-facing messages into a single file `packages/shared/src/messages.ts`.

**Architecture:** Create `packages/shared/src/messages.ts` as the single source of truth for all messages (ERROR_MSG, MSG, AUTH_ERROR_MAP, PUSH_MSG, UI labels). Existing import paths remain valid by converting consumer files to thin re-exports, so no churn in the 60+ files that import messages.

**Tech Stack:** TypeScript, Turborepo monorepo (`packages/shared` → `@sugara/shared`), Vitest

---

## Current state

| File | Export | Count | Language |
|---|---|---|---|
| `apps/api/src/lib/constants.ts` | `ERROR_MSG` | 74 | English |
| `apps/web/lib/messages.ts` | `MSG` | 203 | Japanese |
| `apps/web/lib/auth-error.ts` | `AUTH_ERROR_MAP`, `translateAuthError` | 16+fn | Japanese |
| `apps/api/src/lib/notifications.ts` | hardcoded in `buildPushMessage()` | 9 | Japanese |
| `packages/shared/src/constants.ts` | `*_LABELS` (5 maps) | 29 | Japanese |

After this plan, all of these live in `packages/shared/src/messages.ts`.

---

### Task 1: Create packages/shared/src/messages.ts
...
Expected: all tests pass (note the counts as baseline).

**Step 2: Create messages.ts**

Create `packages/shared/src/messages.ts` with the full content below:
...
Expected: no errors (new file exports, no consumers yet).
...
Expected: all tests pass (no consumers changed yet).

**Step 6: Commit**

```bash
git add packages/shared/src/messages.ts packages/shared/src/index.ts
git commit -m "feat: packages/shared に messages.ts を追加（全メッセージの集約先）"
```

---

### Task 2: Remove *_LABELS from packages/shared/src/constants.ts

`CATEGORY_LABELS`, `TRANSPORT_METHOD_LABELS`, `STATUS_LABELS`, `ROLE_LABELS`, `SCHEDULE_COLOR_LABELS` are now defined in `messages.ts`. Remove them from `constants.ts` to avoid duplicate exports.
...
Remove the 5 label maps AND their type imports from `constants.ts`. The file currently looks like:
...
After edit, `constants.ts` should be:

```typescript
import type { ScheduleCategory } from "./schemas/schedule";
...
Expected: no errors. All consumers of `CATEGORY_LABELS` etc. import from `@sugara/shared` and `index.ts` now re-exports them from `messages.ts`.
...
git commit -m "refactor: *_LABELS を messages.ts に移動、constants.ts から削除"
```

---

### Task 3: Make apps/api/src/lib/constants.ts re-export ERROR_MSG from shared

`ERROR_MSG` is now defined in `@sugara/shared`. Replace the definition with a re-export so all 27+ route files that import from `"../lib/constants"` continue working without changes.
...
Replace the `ERROR_MSG = { ... } as const` block with a re-export line.

The current file ends with `} as const;` on the last line. Remove the entire `export const ERROR_MSG = { ... } as const;` block, then add:

```typescript
export { ERROR_MSG } from "@sugara/shared";
```

The `MAX_LOGS_PER_TRIP` re-export at the top stays as-is. The import at the top:

```typescript
import { MAX_LOGS_PER_TRIP } from "@sugara/shared";
```

stays. The final file should look like:
...
git commit -m "refactor: ERROR_MSG を @sugara/shared から re-export に変更"
```

---

### Task 4: Make apps/web/lib/messages.ts re-export MSG from shared

`MSG` is now defined in `@sugara/shared`. Replace the 275-line definition with a one-line re-export. All 60 web files that import `{ MSG } from "@/lib/messages"` continue working.

**Files:**
- Modify: `apps/web/lib/messages.ts`

**Step 1: Replace messages.ts content**

Replace the entire content of `apps/web/lib/messages.ts` with:

```typescript
export { MSG } from "@sugara/shared";
```
...
git commit -m "refactor: MSG を @sugara/shared から re-export に変更"
```

---

### Task 5: Make apps/web/lib/auth-error.ts re-export from shared

`AUTH_ERROR_MAP` and `translateAuthError` are now in `@sugara/shared`. Replace the definitions with re-exports.

**Files:**
- Modify: `apps/web/lib/auth-error.ts`

**Step 1: Replace auth-error.ts content**

Replace the entire content of `apps/web/lib/auth-error.ts` with:

```typescript
export { translateAuthError } from "@sugara/shared";
```

Note: `AUTH_ERROR_MAP` is not exported from `auth-error.ts` (it's a module-private `const`), only `translateAuthError` is. Verify by checking imports:

```bash
grep -r "from.*auth-error" apps/web/
```

If only `translateAuthError` is imported (expected), the above is correct. If `AUTH_ERROR_MAP` is also imported somewhere, add it: `export { AUTH_ERROR_MAP, translateAuthError } from "@sugara/shared"`.
...
```bash
git add apps/web/lib/auth-error.ts
git commit -m "refactor: translateAuthError を @sugara/shared から re-export に変更"
```

---

### Task 6: Use PUSH_MSG from shared in buildPushMessage()

Replace the hardcoded switch-case in `buildPushMessage()` with `PUSH_MSG` from shared.

**Files:**
- Modify: `apps/api/src/lib/notifications.ts:105-167`

**Step 1: Update notifications.ts**

In `apps/api/src/lib/notifications.ts`, update the import line at the top from:
...
Then replace the entire `buildPushMessage` function (lines 105–167):
...
Expected: no errors. Note: `NotificationPayload` uses `[key: string]: string | undefined` which is compatible with `Record<string, string | undefined>`.
...
git commit -m "refactor: buildPushMessage を PUSH_MSG 定数に置き換え"
```
...
Expected: all tests pass (same counts as baseline in Task 1, Step 1).
...
Confirm that editing `packages/shared/src/messages.ts` is the only place needed to change any user-facing message:
- API errors: `ERROR_MSG` in messages.ts
- UI toasts: `MSG` in messages.ts
- Auth errors: `AUTH_ERROR_MAP` in messages.ts
- Push notifications: `PUSH_MSG` in messages.ts
- UI labels: `*_LABELS` in messages.ts
