...
### Task 2: 共有 Zod スキーマを更新

**Files:**
- Modify: `packages/shared/src/schemas/trip.ts`
- Modify: `packages/shared/src/schemas/schedule.ts`

**Step 1: trip.ts に mapsEnabled を追加**

`packages/shared/src/schemas/trip.ts` で trip のレスポンス型として使われているスキーマを確認する。`tripSchema` または相当する z.object に以下を追加:

```typescript
mapsEnabled: z.boolean(),
```

`createTripSchema`（入力スキーマ）には追加しない（ユーザーが指定するものではないため）。

**Step 2: schedule.ts に lat/lng/placeId を追加**
...
`updateScheduleSchema` が `createScheduleSchema.partial()` から派生しているなら変更不要。独立して定義されている場合は同様に追加。
...
```bash
git add packages/shared/src/schemas/trip.ts packages/shared/src/schemas/schedule.ts
git commit -m "feat: Zod スキーマに mapsEnabled / lat / lng / placeId を追加"
```
...
このファイル自体は現時点では中継のみ。APIProvider は page.tsx のクライアントコンポーネント内で条件付きにレンダリングする（layout はサーバーコンポーネントのため、クライアントライブラリを直接使えない）。

**Step 3: page.tsx で mapsEnabled に応じて APIProvider を条件付きレンダリング**

`apps/web/app/(authenticated)/trips/[id]/page.tsx` で `trip.mapsEnabled` を参照できる箇所を確認し、以下のパターンで `APIProvider` をラップする:

```tsx
// page.tsx（既存のクライアントコンポーネント内）
...
export function PlacesAutocompleteInput({ id, defaultValue, onPlaceSelect }: Props) {
  ...
  useEffect(() => {
    ...
    const autocomplete = new placesLib.Autocomplete(inputRef.current, {
      fields: ["formatted_address", "geometry", "id", "name"],
    })
...
type RightPanelTab = "candidates" | "activity" | "bookmarks" | "expenses" | "souvenirs" | "map"
```

タブのアイコン・ラベル配列（または switch/if）に map を追加:

```tsx
{ id: "map", icon: Map, label: "地図" },
```

`mapsEnabled` が false の場合は map タブを除外するよう、タブリストを `props.mapsEnabled` で filter する。
...
<RightPanel
  ...
  allSchedules={allDays.flatMap(day =>
...
import type { Schedule } from "@sugara/shared"
...
- Modify: `apps/web/app/(authenticated)/trips/[id]/_components/day-timeline.tsx`
...
```bash
git add apps/web/components/travel-time-separator.tsx apps/web/components/travel-time-separator.test.tsx apps/web/app/(authenticated)/trips/[id]/_components/day-timeline.tsx
